...
export default function initHotkey() {
  ...
  hotkeys('backspace, del', (event: KeyboardEvent) => {
    ...
    if (selections.length) {
      ...
      const ins = context.getNodes().filter(node => node.getType() === EdgeType.Edge && node.getPropData('target')?.id === selected?.getId());
      const outs = context.getNodes().filter(node => node.getType() === EdgeType.Edge && node.getPropData('source')?.id === selected?.getId());
